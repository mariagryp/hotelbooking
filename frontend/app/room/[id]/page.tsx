...
//props params contains route parameters like
const getRoomData = async ({ params }: { params: any }) => {
  const res = await fetch(
    /* populate=*: Ensures that all related data (e.g., images, other entities) is included in the API response. */
    `http://127.0.0.1:1337/api/rooms/${params.id}?populate=*`,
    {
      /* next: Configuration for Next.js to control caching behavior, with revalidate(Обновлять): 0 forcing fresh data on each request. */
      next: {
        revalidate: 0,
      },
    }
